...
namespace Scada.Comm.Drivers.DrvSigModbus.View
{
    ...
    public static class DriverPhrases
    {
        ...
        {
            LocaleDict dict = Locale.GetDictionary("Scada.Comm.Drivers.DrvSigModbus.View.Controls");
            AddressHint = dict[nameof(AddressHint)];
            ElemRemoveWarning = dict[nameof(ElemRemoveWarning)];

            dict = Locale.GetDictionary("Scada.Comm.Drivers.DrvSigModbus.View.Forms.FrmDeviceProps");
            ConfigDirRequired = dict[nameof(ConfigDirRequired)];
            TemplateNotExists = dict[nameof(TemplateNotExists)];

            dict = Locale.GetDictionary("Scada.Comm.Drivers.DrvSigModbus.View.Forms.FrmDeviceTemplate");
